...
REM 11.2. Метод получения токена аутентификации в СУЗ необходимо иметь connectionID из ЛК/Устройства
...
curl -X POST "https://markirovka.crpt.ru/api/v3/true-api/auth/simpleSignIn/"%conId% ^
 ...
 -d @%USERPROFILE%"\get_jwt_token.json" >  %USERPROFILE%\token_%conId%.json
