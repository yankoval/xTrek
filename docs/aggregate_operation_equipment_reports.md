...
- `/api/v3/true-api/cises/aggregated/list` — состав агрегата (после расформирования
  может сохраняться прежний состав).
...
После изъятия последнего вложения короб может перейти в `DISAGGREGATION`, а
`/cises/info` — вернуть его код только в `cisInfo.requestedCis`, без `cis`.
Проверка учитывает этот формат: если агрегат найден и запрошенных вложений больше
нет, отчёт получает `check=finished`. Отсутствующий агрегат при изъятии по-прежнему
даёт `aggregatenotfound`; временные ошибки API не изменяют тег.

Если расформированный агрегат сохраняет прежний состав, завершение изъятия
подтверждается по каждому запрошенному коду: статус `APPLIED`, `INTRODUCED`,
`DISAGGREGATION` или `DISAGGREGATED`, а `parent` отсутствует либо указывает на
другой агрегат. Это также допускает последующее расформирование извлечённого
короба. Неизвестный код или сохранившаяся связь с исходным родителем не позволяют
игнорировать состав. Для действующего родителя прежняя проверка не меняется.
...
Если тестовый worker использует штатный промышленный демон подписи, параметр
`sign` указывает на существующий промышленный префикс `sign/`. Это транспортное
исключение: исходные отчёты, задачи, квитанции и финальные статусы продолжают
храниться в тестовом `internal_bucket`, а временные тело и подпись удаляются
после отправки документа. Отдельный sign-бакет и отдельный test sign-trigger для
такой схемы не создаются.
