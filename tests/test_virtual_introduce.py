...
from xtrek.create_emission_task_sample import create_virtual_introduce_task
...
def test_create_virtual_introduce_task_virtual_true(mock_config, mock_storage):
    ...
    with patch('xtrek.create_emission_task_sample._find_production_order_id_by_suz_order_id') as mock_find:
        ...
        # Mock create_introduce_task
        with patch('xtrek.create_emission_task_sample.create_introduce_task') as mock_create:
            mock_create.return_value = "doc-123"

            result = create_virtual_introduce_task(order_id, "group1")

            assert isinstance(result, ProductionOrder)
            assert result.virtual is True
            mock_create.assert_called_once_with(order_id, "group1", None)

def test_create_virtual_introduce_task_virtual_false(mock_config, mock_storage):
    order_id = "suz-order-123"
    prod_order_id = "prod-order-456"
