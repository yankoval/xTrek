...
@patch('xtrek.create_emission_task_sample.TokenProcessor')
@patch('xtrek.create_emission_task_sample.NK')
def test_create_virtual_production_tasks_with_qty_override(mock_nk_class, mock_token_processor_class, mock_org_manager_class, mock_get_inn, mock_get_storage, mock_load_config):
    # Setup mocks
    mock_load_config.return_value = {
        'production_orders_path': 's3://bucket/prod_orders/',
        's3_config': {}
    ...
    mock_nk.feedProduct.side_effect = [
        {"result": [{"is_set": True}]},
        {"result": [{"is_set": False, "good_name": "Comp 1"}]}
    ]
    mock_nk.get_set_by_gtin.return_value = {
        "result": [{
            "is_set": True,
    ...
    # Run with qty=50 override
    create_virtual_production_tasks("source_order", qty=50)
