...
Универсальный отчёт за день (идентификатор `tasks-grouped`). Без `--group-by`
выводит только общие итоги, как отчёт `tasks`:
...
Опция `--group-by` принимает от нуля до двух полей через пробел.
Порядок полей задаёт порядок вложенности:

    # без --group-by          только общие итоги
    --group-by                только общие итоги
    --group-by operator       один уровень: оператор
    --group-by article        один уровень: артикул
    --group-by operator article   оператор → артикул
    --group-by article operator   артикул → оператор

`operator` берётся из `PasportData.operator`, `article` — из `Article`.
Другие поля, повторяющиеся поля и более двух уровней отклоняются до чтения S3.
Запятые не используются. Пример PDF с двумя уровнями:

    python -m xtrek.reports.tasks-grouped \
        --from 2026-08-25T09:30 \
        --to 2026-08-26T12:15 \
        --group-by operator article \
        --format pdf \
        --output tasks-by-operator.pdf

Можно также использовать `--date`, форматы HTML/MD и профиль `messenger`.
В Python отсутствие `group_by`, `None` или пустой список означает только итоги.
Примеры: `collect(source, day="2026-08-25", group_by="operator")` или
`collect_range(..., group_by=["article", "operator"])`, затем `build(data)`.
Также поддерживается строка `group_by="operator article"`.
При одном уровне каждая строка содержит итог по значению поля.
При двух уровнях после вложенных строк каждой группы обязательно выводится
подытог по заданиям, ярлыкам и кодам; в конце отчёта — общий итог.
Подытоги сохраняются и при `--details none`, включая краткий вывод для мессенджера.
Расшифровка файлов следует тому же порядку группировки.
Без группировок расшифровка не выводится даже при `--details full`:
в этом режиме остаётся только сводка за выбранный день или период.
Пустой, отсутствующий или null `PasportData.operator`
отображается как «Без оператора». Пробелы по краям имени удаляются, регистр
и написание сохраняются. Поле `operator` верхнего уровня не используется.
В обеих группировках задания с нулевым `Quantity` исключаются.

В режиме `--group-by article` для каждого значения `Article` выводятся количество заданий, сумма паспортов
